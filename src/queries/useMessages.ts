...
export function useCreateChat() {
  ...
  return useMutation({
    mutationFn: async ({ buyerId, sellerId, listingId }: { buyerId: string; sellerId: string; listingId: string }) => {
      const { data, error } = await createChat(buyerId, sellerId, listingId);
      if (error) throw error;
      return data!;
    },
...
export function useSendMessage() {
  ...
  return useMutation({
    mutationFn: async ({ chatId, content }: { chatId: string; content: string }) => {
      ...
      const { data, error } = await sendMessage(chatId, userId, content);
      if (error) throw error;
      return data!;
    },
    onSuccess: (data) => {
